...
<style>
    ...
    .status-radio input[type="radio"]:checked + .status-radio-content .status-icon {
        color: white;
    }
</style>
@endpush
...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-heading font-bold text-gray-900">Tambah Portofolio</h1>
        <p class="text-gray-500 text-sm mt-1">Tambahkan proyek portofolio baru</p>
    </div>
    ...
    <form action="{{ route('content.portofolio.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8 mb-6">
            ...
            <div class="grid md:grid-cols-2 gap-5">
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-2" id="statusContainer">
                        ...
                        <label class="status-radio relative flex flex-col items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all
                                      {{ old('status_proyek', 'selesai') == $status ? 'active' : 'border-gray-200 hover:border-gray-300' }}
                                      {{ $status == 'berjalan' ? 'berjalan' : '' }}
                                      {{ $status == 'direncanakan' ? 'direncanakan' : '' }}
                                      {{ $status == 'selesai' ? 'selesai' : '' }}"
                               data-status="{{ $status }}">
                            <input type="radio" name="status_proyek" value="{{ $status }}"
                                   {{ old('status_proyek', 'selesai') == $status ? 'checked' : '' }}
                                   class="hidden status-radio-input">
                            <i class="fas fa-{{ $colors[1] }} text-xl status-icon {{ old('status_proyek', 'selesai') == $status ? 'text-'.$colors[0].'-600' : 'text-gray-400' }}"></i>
                ...
                <div class="flex items-center">
                    <label class="relative flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all duration-300 featured-toggle inactive"
                           id="featuredToggle">
                        <input type="checkbox" name="is_featured" value="1" class="sr-only" id="featuredCheckbox">
                        <div class="w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-300 featured-icon bg-gray-100 text-gray-400">
                            <i class="fas fa-star"></i>
                        </div>
                        <div>
                            <span class="text-sm font-semibold transition-colors duration-300 featured-text text-gray-900">Featured</span>
                            <p class="text-xs transition-colors duration-300 featured-desc text-gray-500">Tampilkan di halaman utama</p>
                        </div>
                    </label>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8 mb-6">
            ...
            <textarea name="deskripsi" rows="6" required
                      class="w-full rounded-xl border-gray-200 bg-gray-50 py-2.5 px-4 focus:bg-white focus:border-primary-500 focus:ring-2 focus:ring-primary-100 transition-all text-sm">{{ old('deskripsi') }}</textarea>
        </div>

        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8 mb-6">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-heading font-semibold text-gray-900">Tambah Gambar</h3>
...
document.getElementById('featuredCheckbox')?.addEventListener('change', function() {
    ...
    if (this.checked) {
        ...
        statusDesc.textContent = 'Akan tampil di halaman utama sebagai unggulan';
    } else {
