    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="relative group rounded-xl overflow-hidden bg-gray-100" data-id="{{ $gallery->id }}">
                <img src="{{ storage_url($gallery->image_path) }}" class="w-full h-32 object-cover">
                @if($gallery->caption)
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8 mb-6">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    @if($inspirasi->gambar)
                    <div class="mb-3">
                        <img src="{{ storage_url($inspirasi->gambar) }}" class="w-40 h-28 object-cover rounded-xl border border-gray-200">
                ...
                <div>
                    ...
                    <label class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all duration-300 status-toggle {{ $inspirasi->is_published ? 'active' : 'inactive' }}"
                           id="statusToggle">
                        <input type="checkbox" name="is_published" value="1" class="sr-only" id="statusCheckbox"
                               {{ $inspirasi->is_published ? 'checked' : '' }}>
                        <div class="w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-300 status-icon {{ $inspirasi->is_published ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400' }}">
                            <i class="fas fa-check"></i>
                        </div>
                        <div>
                            <span class="text-sm font-semibold transition-colors duration-300 status-text">{{ $inspirasi->is_published ? 'Published' : 'Publish' }}</span>
                            <p class="text-xs transition-colors duration-300 status-desc">{{ $inspirasi->is_published ? 'Inspirasi sudah tayang di publik' : 'Centang untuk publish' }}</p>
                        </div>
                    </label>
                </div>
            </div>
...
document.getElementById('statusCheckbox')?.addEventListener('change', function() {
    ...
    if (this.checked) {
        ...
        statusDesc.textContent = 'Inspirasi sudah tayang di publik';
    } else {
